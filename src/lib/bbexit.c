...
static void
unpack (unsigned int coord, int *index, int *x, int *y)
{
  static union
  {
    int x;
...
static void
profout (struct _bbdata *p, FILE * fp)
{
  int i, index, x, y;
  struct func *f;
  struct caller *q;
...
EXPORT void
_epilogue (struct func *callee)
{
  *_callerp = 0;
}

EXPORT void
_prologue (struct func *callee, struct _bbdata *yylink)
{
