 ...
 */
static int
emittype (Type ty, int lev, int col)
{
  int tc = ty->x.typeno;
  ...
  switch (ty->op)
    ...
    case INT:
      if (ty == chartype)       /* plain char is a subrange of itself */
        print ("=r%d;%d;%d;", tc, ty->u.sym->u.limits.min.i,
               ty->u.sym->u.limits.max.i), col +=
          2 + 3 + 2 * 2.408 * ty->size + 2;
      else                      /* other signed ints are subranges of int */
        print ("=r1;%D;%D;", ty->u.sym->u.limits.min.i,
               ty->u.sym->u.limits.max.i), col +=
          4 + 2 * 2.408 * ty->size + 2;
      break;
    case UNSIGNED:
    ...
    case UNION:
      {
        ...
        print ("=%c%d", ty->op == STRUCT ? 's' : 'u', ty->size), col +=
          1 + 1 + 3;
        for (p = fieldlist (ty); p; p = p->link)
          {
            if (p->name)
    ...
    case ENUM:
      {
        ...
        for (p = ty->u.sym->u.idlist; *p; p++)
          {
            print ("%s:%d,", (*p)->name, (*p)->u.value), col +=
              strlen ((*p)->name) + 3;
            if (col >= 80 && p[1])
              {
                print ("\\\\\",%d,0,0,0\n.stabs \"", N_LSYM);
...
stabblock (int brace, int lev, Symbol * p)
{
  ...
  if (IR == &sparcIR)
  ...
  else
    {
      ...
      print (".stabn 0x%x,0,%d,%s%d-%s\n", brace == '{' ? N_LBRAC : N_RBRAC,
             lev, stabprefix, lab, cfunc->x.name);
      print ("%s%d:\n", stabprefix, lab);
    }
}
  ...
  if (p->sclass == AUTO && p->scope == GLOBAL || p->sclass == EXTERN)
  ...
  else if (p->sclass == STATIC)
    {
      print (".stabs \"%s:%c%d\",%d,0,0,%s\n", p->name,
             ...
             p->u.seg == BSS ? N_LCSYM : N_STSYM, p->x.name);
      return;
    }
...
void
stabfend (Symbol p, int lineno)
{
}

/* stabend - finalize stab output */
void
stabend (Coordinate * cp, Symbol p, Coordinate ** cpp, Symbol * sp,
         Symbol * stab)
{
  (*IR->segment) (CODE);
