...
static struct table cns = { CONSTANTS }, ext =
...
void
foreach (Table tp, int lev, void (*apply) (Symbol, void *), void *cl)
{
  assert (tp);
...
Symbol
install (const char *name, Table * tpp, int level, int arena)
{
  Table tp = *tpp;
  struct entry *p;
...
Symbol
relocate (const char *name, Table src, Table dst)
{
  struct entry *p, **q;
  Symbol *r;
  unsigned h = (unsigned long) name & (HASHSIZE - 1);
...
Symbol
lookup (const char *name, Table tp)
{
  struct entry *p;
  unsigned h = (unsigned long) name & (HASHSIZE - 1);
...
Symbol
genident (int scls, Type ty, int lev)
{
  Symbol p;
...
Symbol
newtemp (int sclass, int tc, int size)
{
...
Symbol
mksymbol (int sclass, const char *name, Type ty)
{
  Symbol p;
