...
static void
bbcall (Symbol yycounts, Coordinate * cp, Tree * e)
{
...
static void
bbentry (Symbol yylink, Symbol f, void *ignore)
{
  ...
  if (prologue == 0)
    {
      prologue =
        mksymbol (EXTERN, "_prologue",
                  ftype (inttype, voidptype, voidptype, NULL));
      prologue->defined = 0;
    }
  walk (vcall
        (prologue, voidtype, pointer (idtree (afunc)),
         pointer (idtree (yylink)), NULL), 0, 0);
}

/* bbexit - return tree for _epilogue(&afunc)' */
static void
bbexit (Symbol yylink, Symbol f, Tree e)
{
  ...
  if (epilogue == 0)
    {
      epilogue =
        mksymbol (EXTERN, "_epilogue", ftype (inttype, voidptype, NULL));
      epilogue->defined = 0;
    }
...
static void
bbfunc (Symbol yylink, Symbol f, void *ignore)
{
  Value v;
  union coordinate u;
...
static void
bbincr (Symbol yycounts, Coordinate * cp, Tree * e)
{
...
static void
bbvars (Symbol yylink, void *ignore, void *ignore2)
{
  int i, j, n = npoints;
  Value v;
  struct map **mp;
...
profInit (char *arg)
{
  if (strncmp (arg, "-a", 2) == 0)
    {
      if (ncalled == -1 && process (arg[2] ? &arg[2] : "prof.out") > 0)
        ncalled = 0;
    }
