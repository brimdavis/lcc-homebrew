...
Tree
call (Tree f, Type fty, Coordinate src)
{
  int n = 0;
...
Tree
calltree (Tree f, Type ty, Tree args, Symbol t3)
{
  Tree p;
  ...
  if (isstruct (ty))
    assert (t3),
      p = tree (RIGHT, ty,
                tree (CALL + B, ty, f, addrof (idtree (t3))), idtree (t3));
  else
    {
      Type rty = ty;
  ...
  if (l->op == FIELD)
    {
      ...
      if (n > 0 && isunsigned (l->u.field->type))
        r = bittree (BAND, r,
                     cnsttree (r->type,
                               (unsigned long) fieldmask (l->u.field)));
      else if (n > 0)
        {
          if (r->op == CNST + I)
            {
...
void
typeerror (int op, Tree l, Tree r)
{
  int i;
  static struct
  {
