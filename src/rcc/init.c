...
static int
initarray (int len, Type ty, int lev)
{
  int n = 0;
  ...
  do
    {
      ...
      if (fieldsize (p) < 8 * p->type->size)
        {
          if (p->type == inttype &&
              (i < -(int) (fieldmask (p) >> 1) - 1
               || i > (int) (fieldmask (p) >> 1)) || p->type == unsignedtype
              && (i & ~fieldmask (p)) != 0)
...
static int
initstruct (int len, Type ty, int lev)
{
  int a, n = 0;
