...
void
statement (int loop, Swtch swp, int lev)
{
  float ref = refinc;
...
static void
ifstmt (int lab, int loop, Swtch swp, int lev)
{
  t = gettok ();
  expect ('(');
  definept (NULL);
...
static void
forstmt (int lab, Swtch swp, int lev)
{
  int once = 0;
...
static void
swstmt (int loop, int lab, int lev)
{
  Tree e;
  struct swtch sw;
  Code head, tail;
...
static void
caselabel (Swtch swp, long val, int lab)
{
  int k;
...
void
swcode (Swtch swp, int b[], int lb, int ub)
{
  ...
  if (u - l + 1 <= 3)
  ...
  else
    {
      ...
      cmp (GT, swp->sym, v[u], hilab);
      e =
        (*optree['-']) (SUB, cast (idtree (swp->sym), ty),
                        cnsttree (ty, v[l]));
      if (e->type->size < signedptr->size)
        e = cast (e, longtype);
      walk (tree (JUMP, voidtype,
                  rvalue ((*optree['+']) (ADD, pointer (idtree (table)), e)),
                  NULL), 0, 0);
      code (Switch);
...
static void
cmp (int op, Symbol p, long n, int lab)
{
...
void
equatelab (Symbol old, Symbol new)
{
...
static int
equal (Symbol lprime, Symbol dst)
{
  assert (dst && lprime);
...
static void
dostmt (int lab, Swtch swp, int lev)
{
  refinc *= 10.0;
  t = gettok ();
  definelab (lab);
...
foldcond (Tree e1, Tree e2)
{
  ...
  if ((op == LE || op == LT || op == EQ || op == NE || op == GT || op == GE)
      ...
      && e2->kids[0]->kids[0]->u.sym == v && e2->kids[1]->op == e1->op)
    {
...
static void
whilestmt (int lab, Swtch swp, int lev)
{
  Coordinate pt;
  Tree e;
