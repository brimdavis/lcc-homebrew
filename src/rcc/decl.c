...
static void
decl (Symbol (*dcl) (int, char *, Type, Coordinate *))
{
  int sclass;
  Type ty, ty1;
...
static Symbol
dclglobal (int sclass, char *id, Type ty, Coordinate * pos)
{
  Symbol p;
...
static Type
dclr (Type basety, char **id, Symbol ** params, int abstract)
{
...
static Type
dclr1 (char **id, Symbol ** params, int abstract)
{
  Type ty = NULL;
...
static Symbol
dclparam (int sclass, char *id, Type ty, Coordinate * pos)
{
  Symbol p;
...
static void
funcdefn (int sclass, char *id, Type ty, Symbol params[], Coordinate pt)
{
  int i, n;
          ...
          for (i = 0; caller[i] && proto[i]; i++)
            {
              ...
              if (eqtype (isenum (ty) ? ty->type : ty,
                ...
                warning
                  ("compatibility of `%t' and `%t' is compiler dependent\n",
...
void
compound (int loop, struct swtch *swp, int lev)
{
  Code cp;
  int nregs;
...
checkref (Symbol p, void *cl)
{
  ...
  assert (!
          (level == GLOBAL && p->sclass == STATIC && !p->defined
           && !isfunc (p->type)));
}
static Symbol
dcllocal (int sclass, char *id, Type ty, Coordinate * pos)
{
  Symbol p, q;
  ...
  assert (level >= LOCAL);
  p = install (id, &identifiers, level, sclass == STATIC
               || sclass == EXTERN ? PERM : FUNC);
  p->type = ty;
  p->sclass = sclass;
  p->src = *pos;
  switch (sclass)
...
static void
doextern (Symbol p, void *cl)
{
  (*IR->import) (p);
}
static void
doglobal (Symbol p, void *cl)
{
