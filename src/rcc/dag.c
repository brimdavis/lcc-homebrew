...
void
walk (Tree tp, int tlab, int flab)
{
...
static Node
node (int op, Node l, Node r, Symbol sym)
{
  int i;
  struct dag *p;
...
static struct dag *
dagnode (int op, Node l, Node r, Symbol sym)
{
  struct dag *p;
...
Node
newnode (int op, Node l, Node r, Symbol sym)
{
...
Node
listnodes (Tree tp, int tlab, int flab)
{
  Node p = NULL, l, r;
  int op;
    ...
    case FIELD:
      {
        ...
        if (tp->type == inttype)
          ...
          q = bittree (BAND,
                       shtree (RSH, q,
                               cnsttree (inttype,
                                         (long) fieldright (tp->u.field))),
                       cnsttree (unsignedtype,
                                 (unsigned long) fieldmask (tp->u.field)));
        assert (tlab == 0 && flab == 0);
        p = listnodes (q, 0, 0);
      } break;
    case ADDRG:
...
void
gencode (Symbol caller[], Symbol callee[])
{
  Code cp;
  Coordinate save;
...
tmpnode (Node p)
{
  ...
  p = newnode (INDIR + ttob (tmp->type),
               newnode (ADDRL + ttob (voidptype), NULL, NULL, tmp), NULL,
               NULL);
  p->count = 1;
  return p;
}
static Node
asgnnode (Symbol tmp, Node p)
{
...
static void
printdag1 (Node p, int fd, int lev)
{
  int id, i;
...
static void
printnode (Node p, int fd, int lev)
{
  if (p)
    {
      FILE *f = fd == 1 ? stdout : stderr;
