...
static void
emitString (int len, const char *s)
{
  for (; len-- > 0; s++)
    if (*s == '&' && html)
      print ("&amp;");
...
#define yy(f) if (p->f) { if (n++) print("|"); print(#f); }
  yy (structarg) yy (addressed) yy (computed) yy (temporary) yy (generated)
#undef yy
    if (n == 0)
    print ("0");
  END;
  ...
  if (html)
    {
      ...
      BEGIN (em);
      print
        ("Links lead from uses of identifiers and labels to their definitions.");
      END;
      END;
      print ("<ul>\n");
      START;
...
static void I (stabend) (Coordinate * cp, Symbol p, Coordinate ** cpp,
                         Symbol * sp, Symbol * stab)
{
  int i;
