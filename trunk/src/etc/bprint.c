...
void *
allocate (unsigned long n, unsigned a)
{
  return alloc (n);
}

void *
newarray (unsigned long m, unsigned long n, unsigned a)
{
  return alloc (m * n);
}
            ...
            if (q->count.count == 0 || !q->callers)
              ...
              for (r = q->callers; r; r = r->link)
                fprintf (fp, "%s 1 %d %d %d %s %s %d %d\n", q->name,
                         q->count.x, q->count.y, r->count, r->name, r->file,
                         r->x, r->y);
          fprintf (fp, "%d\n", p->count);
          for (i = 0; i < p->count; i++)
            fprintf (fp, "1 %d %d %d\n", p->counts[i].x,
...
void
printfile (struct file *p, int nf)
{
  int lineno;
  FILE *fp;
  char *s, buf[512];
...
void
printfuncs (struct file *p, int nf)
{
  struct func *q;
