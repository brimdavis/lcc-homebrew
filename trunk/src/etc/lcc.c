...
main (int argc, char *argv[])
{
  ...
  ilist = 0;
  for (i = 1; argv[i]; i++)
    if (strcmp (argv[i], "-l") == 0 && argv[i + 1] && *argv[i + 1] != '-')
      {                         /* -l file */
  ...
  if (errcnt == 0 && !Eflag && !Sflag && !cflag && llist[1])
    {
      compose (ld, llist[0], llist[1],
               append (outfile ? outfile : concat ("a", first (suffixes[4])),
                       0));
      if (callsys (av))
        errcnt++;
    }
...
static void
compose (char *cmd[], List a, List b, List c)
{
  int i, j;
  List lists[3];
  ...
  switch (suffix (name, suffixes, 4))
    ...
    case 1:                    /* preprocessed source files */
      ...
      if (Sflag)
        status =
          compile (name,
                   outfile ? outfile : concat (base, first (suffixes[2])));
      else
        if ((status =
             compile (name,
                      stemp ? stemp : (stemp =
                                       tempname (first (suffixes[2]))))) == 0)
...
#define xx(name,k) \
            ...
            xx (unsigned_char, 1) xx (unsigned_short, 2) xx (unsigned_int, 4)
#undef xx
              return;
          case 'a':
...
char *
replace (const char *str, int from, int to)
{
...
int
suffix (char *name, char *tails[], int n)
{
