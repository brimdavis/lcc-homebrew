...
Tree
tree (int op, Type type, Tree left, Tree right)
{
  Tree p;
...
Tree
texpr (Tree (*f) (int), int tok, int a)
{
  int save = where;
  Tree p;
...
root1 (Tree p)
{
  ...
  switch (generic (p->op))
    ...
    case CNST:
      ...
      return NULL;
    case CVF:
      if (optype (p->op) == I || p->type->size < p->kids[0]->type->size)
        if (warn++ == 0)
          warning ("expression with no effect elided\n");
      return root1 (p->kids[0]);
    case CVI:
...
static void
printtree1 (Tree p, int fd, int lev)
{
