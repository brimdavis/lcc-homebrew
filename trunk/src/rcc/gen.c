...
unsigned vmask[2];
Symbol
mkreg (char *fmt, int n, int mask, int set)
{
  Symbol p;
...
void
blkcopy (int dreg, int doff, int sreg, int soff, int size, int tmp[])
{
  assert (size >= 0);
  if (size == 0)
    return;
...
static void
blkunroll (int k, int dreg, int doff, int sreg, int soff, int size, int tmp[])
{
  int i;
...
static void
dumpcover (Node p, int nt, int in)
{
  int rulenum, i;
  short *nts;
  Node kids[10];
...
void
setreg (Node p, Symbol r)
{
  p->syms[RX] = r;
}

void
rtarget (Node p, int n, Symbol r)
{
  Node q = p->kids[n];
...
//           )
  ...
  setreg (q, r);
  debug (fprint
         (stderr, "(targeting %x->x.kids[%d]=%x to %s)\n", p, n, p->kids[n],
          r->x.name));
}
...
//
// BMD lcc book says there's a second pass over the forest here to 
// retarget common subexpressions to dst regs; gone missing in 4.2???
//
...
//printf(" putreg: freeing r->x.name = %s\n", r->x.name         ); //BMD
...
static Symbol
askreg (Symbol rs, unsigned rmask[])
{
  int i;
...
static Symbol
getreg (Symbol s, unsigned mask[], Node p)
{
...
int
askregvar (Symbol p, Symbol regs)
{
  Symbol r;
  ...
  // BMD '?' also used for temporary name
  else if (p->temporary)
    {
      p->x.name = "?";
      return 1;
...
//printf(" p->x.registered : %d\n", p->x.registered          ); //BMD
//printf(" NeedsReg        : %d\n", NeedsReg[opindex(p->op)] ); //BMD
...
//BMD  printf(" ralloc : masking %s from node %x)\n", r->x.name, (char *) p); //BMD
...
// printf("exiting ralloc if sclass \n"); // BMD
...
static Symbol
spillee (Symbol set, unsigned mask[], Node here)
{
  Symbol bestreg = NULL;
  int bestdist = -1, i;
      ...
      for (i = 31; i >= 0; i--)
        {
          ...
          if (ri != NULL &&
              ri->x.lastuse &&
              (ri->x.regnode->
               mask & tmask[ri->x.regnode->set] & mask[ri->x.regnode->set]))
            {
...
static void
genspill (Symbol r, Node last, Symbol tmp)
{
  Node p, q;
  Symbol s;
  unsigned ty;
...
static void
genreload (Node p, Symbol tmp, int i)
{
  Node q;
  int ty;

  debug (fprint
         (stderr, "(replacing %x with a reload from %s)\n", p->x.kids[i],
          tmp->x.name));
...
static int
reprune (Node * pp, int k, int n, Node p)
{
  struct node x, *q = *pp;
  ...
  if (k == n)
    {
      debug (fprint
             (stderr, "(reprune changes %x from %x to %x)\n", pp, *pp,
              p->x.kids[n]));
      *pp = p->x.kids[n];
      x = *p;
      (IR->x.target) (&x);
...
void
spill (unsigned mask, int n, Node here)
{
  int i;
  Node p;
...
static void
dumpregs (char *msg, char *a, char *b)
{
