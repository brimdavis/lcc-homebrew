...
expr1 (int tok)
{
  ...
  if (t == '='
      || (prec[t] >= 6 && prec[t] <= 8) || (prec[t] >= 11 && prec[t] <= 13))
    {
      int op = t;
          ...
          if (isunsigned (ty))
            {
              ...
              p =
                simplify (ADD, ty, simplify (BCOM, ty, p, NULL),
                          cnsttree (ty, 1UL));
            }
          else
  ...
  for (;;)
    switch (t)
      ...
      case INCR:
        p = tree (RIGHT, p->type,
                  tree (RIGHT, p->type,
                        p, incr (t, p, consttree (1, inttype))), p);
        t = gettok ();
        break;
      case DECR:
        p = tree (RIGHT, p->type,
  ...
  switch (t)
    ...
    case SCON:
      if (ischar (tsym->type->type))
        ...
        tsym->u.c.v.p =
          memcpy (allocate
                  ((tsym->type->size / widechar->size) * sizeof (int), PERM),
                  tsym->u.c.v.p,
                  (tsym->type->size / widechar->size) * sizeof (int));
...
hascall (Tree p)
{
  ...
  if (generic (p->op) == CALL || (IR->mulops_calls &&
                                  (p->op == DIV + I || p->op == MOD + I
                                   || p->op == MUL + I || p->op == DIV + U
                                   || p->op == MOD + U || p->op == MUL + U)))
    return 1;
  return hascall (p->kids[0]) || hascall (p->kids[1]);
}

Type
binary (Type xty, Type yty)
{
              ...
              if (isunsigned (dst))
                {
                  ...
                  Tree c =
                    cast (cnsttree
                          (longdouble,
                           (long double) sdst->u.sym->u.limits.max.i + 1),
                          src);
                  p =
                    condtree (simplify (GE, src, p, c),
                              (*optree['+']) (ADD,
                                              cast (cast
                                                    (simplify
                                                     (SUB, src, p, c), sdst),
                                              ...
                                              cast (cnsttree
                                                    (unsignedlong,
                                                     (unsigned long) sdst->u.
                                                     sym->u.limits.max.i + 1),
                                                    dst)), simplify (CVF,
                                                                     sdst, p,
                                                                     NULL));
                }
...
Tree
nullcall (Type pty, Symbol f, Tree p, Tree e)
{
  Type ty;
  ...
  return vcall (f, pty,
                ...
                (file
                 && *file ? pointer (idtree (mkstr (file)->u.c.loc)) :
                 cnsttree (voidptype, NULL)), cnsttree (inttype,
                                                        (long) lineno), NULL);
}
