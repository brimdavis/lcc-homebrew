...
static Type
xxinit (int op, char *name, Metrics m)
{
...
static Type
type (int op, Type ty, int size, int align, void *sym)
{
...
#define xx(v,t) if (v==NULL && t->size==voidptype->size && t->align==voidptype->align) v=t
  ...
  if (unsignedptr == NULL)
    unsignedptr =
      type (UNSIGNED, NULL, voidptype->size, voidptype->align,
            voidptype->u.sym);
  ...
  if (signedptr == NULL)
    signedptr =
      type (INT, NULL, voidptype->size, voidptype->align, voidptype->u.sym);
#undef xx
...
#define xx(name,type) \
    ...
    xx (unsigned_char, unsignedchar)
        xx (unsigned_int, unsignedtype) xx (unsigned_short, unsignedshort)}
#undef xx
}
void
rmtypes (int lev)
...
Type
func (Type ty, Type * proto, int style)
{
...
Field
newfield (char *name, Type ty, Type fty)
{
...
int
eqtype (Type ty1, Type ty2, int ret)
{
  if (ty1 == ty2)
    return 1;
  if (ty1->op != ty2->op)
...
Field
fieldref (const char *name, Type ty)
{
...
static Field
isfield (const char *name, Field flist)
{
...
void
printproto (Symbol p, Symbol callee[])
{
  if (p->type->u.f.proto)
    printdecl (p, p->type);
  else
...
static void
prtype (Type ty, FILE * f, int indent, unsigned mark)
{
  switch (ty->op)
    {
    default:
  ...
  for (; ty; ty = ty->type)
    {
      ...
      switch (ty->op)
        ...
        case INT:
        case UNSIGNED:
          return *str ? stringf ("%s %s", ty->u.sym->name,
                                 str) : ty->u.sym->name;
        case POINTER:
          if (!ischar (ty->type) && (p = findtype (ty)) != NULL)
            return *str ? stringf ("%s %s", p->name, str) : p->name;
          str = stringf (isarray (ty->type)
                         || isfunc (ty->type) ? "(*%s)" : "*%s", str);
          break;
        case FUNCTION:
          if ((p = findtype (ty)) != NULL)
            return *str ? stringf ("%s %s", p->name, str) : p->name;
